    ...
    <div class="t-grid t-w-full">
        <div class="t-w-full" v-if="department != ''" >
           <div v-if="lists.length > 0" >
                <div v-for="list in lists" @click="openModal_readSchedule(list.classroomID)" class="t-cursor-pointer t-p-2 t-w-full sm:t-w-[300px]  t-text-white  t-inline-block"  >
                    <div class=" t-p-3  t-bg-logoBlue t-rounded-[5px]" >
                        <div class="t-grid t-items-start" >
                            <h5 class=" text-left t-truncate text-left t-capitalize"  >{{ list.classroom }}</h5>
                            <small class="text-left  t-uppercase" >{{ list.type }}</small>
                        </div>
                    </div>
                </div>
           </div>
