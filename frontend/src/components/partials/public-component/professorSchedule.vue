        ...
        <div class="t-w-full" v-if="department != ''" >
           <div v-if="lists.length > 0" >
                <div v-for="list in lists" @click="openModal_readSchedule(list.professorID)" class="t-cursor-pointer t-p-2 t-w-full sm:t-w-[300px]  t-text-white  t-inline-block"  >
                    <div class="t-grid t-grid-cols-[80px,1fr] t-p-3  t-bg-logoBlue t-rounded-[5px]" >
                        <div>
                            <img src="../../../assets/images/profile.png" class="t-h-[60px]" >
                        </div>
                        <div class="t-grid t-items-start" >
                            <h5 class=" text-left t-truncate text-left t-capitalize" :title="list.fullname.toUpperCase()" >{{ list.fullname }}</h5>
                            <small class="text-left  t-uppercase" >{{ list.professorID }}</small>
                        </div>
                    </div>
                </div>
           </div>
